...
from .feature_3dgs_metrics import Feature3DGSMetrics, Feature3DGSMetricImpl
...
class SimilarityRegularizationMixin:
    def get_similarity_metric(self, gaussian_model, renderer):
        ...
        appearance_features = renderer.features[knn_results.idx.squeeze()]
    ...
    def similarity_metric_backward_hook(self, outputs, batch, gaussian_model, step, pl_module, renderer):
        if step < self.config.similarity_reg_from:
            return
        if step % self.config.similarity_reg_interval != 0:
            return

        similarities_reg, meta, _ = self.get_similarity_metric(gaussian_model, renderer)
...
@dataclass
class Feature3DGSMetricsWithSimilarityRegularization(SimilarityRegularizationConfigMixin, Feature3DGSMetrics):
    def instantiate(self, *args, **kwargs) -> "Feature3DGSMetricsWithSimilarityRegularizationModule":
        return Feature3DGSMetricsWithSimilarityRegularizationModule(self)


class Feature3DGSMetricsWithSimilarityRegularizationModule(SimilarityRegularizationMixin, Feature3DGSMetricImpl):
    pass
